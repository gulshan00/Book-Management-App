...
const Dashboard: React.FC = () => {
  ...
  const availabilityRate = totalBooks > 0 ? Math.round((availableBooks / totalBooks) * 100) : 0;
  ...
  const StatCard: React.FC<{
    ...
    <div className={`${bgColor} backdrop-blur-sm border border-slate-700/50 rounded-xl p-6 shadow-xl transform hover:scale-105 transition-all duration-200 hover:shadow-2xl`}>
      <div className="flex items-center justify-between">
        ...
        <div className={`w-12 h-12 ${color.replace('text-', 'bg-').replace('-400', '-500/20')} rounded-lg flex items-center justify-center`}>
          <span className="text-2xl">{icon}</span>
        </div>
      </div>
    </div>
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <StatCard
          ...
          subtitle={`${availabilityRate}% of collection`}
